  ...
  const tenants: Array<{icon: IconName; title: string; description: string}> = [
    {
      ...
      description:
        'Every mention of Trogdor is an offering to the dragon. Tweet, post, create - and earn points for your devotion.',
    },
    {
      icon: 'crown',
  ...
  return (
    <section className="py-20 px-4 bg-sketch-light">
      <div className="container mx-auto max-w-6xl">
        ...
        <p className="text-center text-pencil mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {tenants.map((tenant) => (
            ...
            >
              ...
              <h3 className="font-hand text-3xl mb-4">{tenant.title}</h3>
              <p className="text-pencil leading-relaxed">{tenant.description}</p>
            </div>
          ))}
        </div>
      </div>
